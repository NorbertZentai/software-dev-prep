...
export class StorageManager {
  ...
  getCompletedCount() {
    const progress = this.getProgress();
    return Object.values(progress).filter(p => p.completionPercentage >= 100).length;
  }
  ...
  getQuizResults(quizTitle = null) {
    const results = JSON.parse(localStorage.getItem(this.keys.QUIZ_RESULTS) || '[]');
  ...
  importData(jsonData) {
    try {
      ...
      if (data.quizResults) {
        localStorage.setItem(this.keys.QUIZ_RESULTS, JSON.stringify(data.quizResults));
      }
      ...
      if (data.studySessions) {
        localStorage.setItem(this.keys.STUDY_SESSIONS, JSON.stringify(data.studySessions));
      }
      ...
      if (data.notes) {
        localStorage.setItem('software_dev_prep_notes', JSON.stringify(data.notes));
      }
  ...
  getStatistics() {
    ...
    const completedRoutes = Object.values(progress).filter(p => p.completionPercentage >= 100).length;
    const totalStudyTime = studySessions.reduce((sum, session) => sum + (session.duration || 0), 0);
    const totalQuizzes = quizResults.length;
    const averageQuizScore = totalQuizzes > 0 ? 
      Math.round(quizResults.reduce((sum, result) => sum + result.percentage, 0) / totalQuizzes) : 0;
    
    return {
      totalRoutes,
      completedRoutes,
      completionPercentage: totalRoutes > 0 ? Math.round((completedRoutes / totalRoutes) * 100) : 0,
      totalStudyTime,
      totalQuizzes,
      averageQuizScore,
  ...
  getLastActivity() {
    ...
    const progressDates = Object.values(progress).map(p => new Date(p.lastVisited));
