...
export class QuizEngine {
      ...
      <div class="quiz-container">
        <header class="quiz-header">
          ...
          <div class="quiz-meta">
            ...
            <span class="estimated-time">~${Math.ceil(quiz.questions.length * 1.5)} perc</span>
          </div>
        </header>
    ...
    return `
      <div class="quiz-question-container">
        <div class="quiz-progress">
          ...
          <span class="progress-text">${questionIndex + 1} / ${this.currentQuiz.questions.length}</span>
        </div>
        ...
        <div class="question-card">
          ...
          <div class="question-actions">
            ${questionIndex > 0 ? '<button id="prev-question" class="btn-secondary">← Előző</button>' : ''}
            <button id="next-question" class="btn-primary" disabled>
              ${questionIndex === this.currentQuiz.questions.length - 1 ? 'Befejezés' : 'Következő →'}
            </button>
          </div>
        </div>
      </div>
  ...
  renderResults() {
    ...
    const resultsHtml = this.currentQuiz.questions.map((question, index) => {
        ...
        <div class="result-item ${isCorrect ? 'correct' : 'incorrect'}">
          ...
          <div class="result-answers">
            <div class="user-answer">
              <strong>Te válaszod:</strong> ${this.formatAnswer(question, userAnswer)}
            </div>
            ${!isCorrect ? `
              <div class="correct-answer">
                <strong>Helyes válasz:</strong> ${this.formatCorrectAnswer(question)}
              </div>
            ` : ''}
          </div>
          ...
          <div class="score-display">
            ...
            <div class="score-details">
              <div class="score-item">
                ...
                <span class="score-value">${correctAnswers} / ${this.currentQuiz.questions.length}</span>
              </div>
              <div class="score-item">
  ...
  formatAnswer(question, userAnswer) {
    if (question.type === 'multiple') {
      ...
      return userAnswer === 'true' ? 'Igaz' : userAnswer === 'false' ? 'Hamis' : 'Nem válaszolt';
    }
    return 'Ismeretlen';
  }
  ...
  getScoreMessage(percentage) {
    ...
    if (percentage >= 70) return '👍 Jó eredmény! Még van mit tanulni, de jó úton jársz.';
    if (percentage >= 60) return '📚 Átlagos eredmény. Érdemes még tanulmányozni a témát.';
    if (percentage >= 40) return '💪 Kezdő szint. Ne add fel, a gyakorlás teszi a mestert!';
